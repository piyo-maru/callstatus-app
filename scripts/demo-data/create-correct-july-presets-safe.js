...
async function createCorrectJulyPresetsSafe() {
  try {
    console.log('=== 正しい7月月次プランナーPendingプリセット作成開始（安全版） ===');
    ...
    // 平日の詳細を表示（最初の5日だけ）
    console.log('平日一覧（最初の5日）:');
    workdays.slice(0, 5).forEach(wd => {
    ...
    const targetTotal = Math.round(totalSlots * targetDensity);
    
    console.log(`\n=== 正しいPendingプリセット作成開始（1/5確率） ===`);
    console.log(`総スロット数: ${totalSlots}`);
    console.log(`期待設定数: 約${targetTotal}件`);
    console.log(`追加目標: ${Math.max(0, targetTotal - currentCount)}件`);
    
    // 各スタッフ × 各平日の組み合わせでランダム処理
    for (const staffMember of staff) {
      for (const workday of workdays) {
        // 重複チェック
        const key = `${staffMember.id}-${workday.dateString}`;
      ...
      if (staffMember.id % 50 === 0) {
        console.log(`  進捗: スタッフID ${staffMember.id}まで処理完了（現在${addedCount}件追加）`);
      }
    }
    ...
    console.log(`最終件数: ${finalCount}件 / ${totalSlots}件 (${finalDensity}%)`);
    ...
    // 曜日別分布確認
    const allSchedules = await prisma.adjustment.findMany({
      where: {
        date: {
          gte: new Date('2025-07-01T00:00:00.000Z'),
          lt: new Date('2025-08-01T00:00:00.000Z')
    ...
    if (weekendCount > 0) {
      console.log(`\n⚠️  土日に${weekendCount}件のスケジュールが存在しています！`);
    } else {
      console.log(`\n✅ 土日にスケジュールはありません。`);
    }
