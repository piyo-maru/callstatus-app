...
async function createJulyTestData() {
  try {
    console.log('7月テストデータ作成開始...');
    ...
    if (staffList.length === 0) {
      console.log('❌ スタッフデータが存在しません。先に社員情報をインポートしてください。');
      return;
    }
    ...
    // 既存の7月申請予定をチェック
    const existingCount = await prisma.adjustment.count({
      where: {
        date: {
    ...
    console.log(`既存の7月申請予定数: ${existingCount}`);
