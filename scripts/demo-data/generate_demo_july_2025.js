...
/**
 * 7/7-7/31 システムプリセット準拠のデモ申請予定と担当設定生成スクリプト
...
// デモ期間設定（7/7から7/31まで）
const START_DATE = new Date('2025-07-07'); 
...
// メイン生成関数
function generateJulyDemoData() {
  const applications = [];
  const responsibilities = [];
  
  console.log('🚀 7月デモデータ生成開始...');
...
if (require.main === module) {
  const demoData = generateJulyDemoData();
  
  const filename = 'demo_data_july_2025.json';
  fs.writeFileSync(filename, JSON.stringify(demoData, null, 2));
  
  console.log(`\n💾 ファイル保存完了: ${filename}`);
  console.log('🎉 7月デモデータ生成完了！');
}

module.exports = { generateJulyDemoData };