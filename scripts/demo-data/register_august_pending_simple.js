...
// メイン実行関数
async function registerAugustPendingSimple() {
  console.log('🚀 8月申請予定登録開始（手動登録と同じ単一スケジュール形式）...');
  ...
  if (totalErrors === 0) {
    console.log('🎉 全ての8月申請予定・担当設定が手動登録と同じ形式で正常に登録されました！');
    console.log('\n✅ 手動登録形式での成功:');
...
// 実行
registerAugustPendingSimple().catch(console.error);