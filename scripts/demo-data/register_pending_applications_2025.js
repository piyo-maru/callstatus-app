...
/**
 * 2025年7-9月申請予定（pending）投入スクリプト
 * 月次計画ページのAPI（/api/schedules/pending）を使用
 */
...
async function registerPendingApplications(applications) {
  ...
  if (errors.length > 0) {
    ...
    errors.slice(0, 5).forEach((err, idx) => {
      console.log(`${idx + 1}. スタッフ${err.application.staffId} ${err.application.date}: ${err.error}`);
    });
  }
...
async function registerAllPendingData() {
  console.log('🚀 2025年7-9月申請予定（pending）投入開始...\n');
