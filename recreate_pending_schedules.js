...
const statusMapping = {
  ...
  '在宅': 'Remote',
  '出張': 'BusinessTrip',
  '振休': 'CompensatoryLeave',
  '振出': 'MakeupWork',
  '通院後出社': 'Medical'
};

// 時間パターンの解析
function parseTimePattern(cellValue) {
  ...
  if (value.includes('退社')) {
    ...
    if (timeMatch) {
      ...
      return {
        ...
        memo: `${hour}:${minute.toString().padStart(2, '0')}退社`
      };
    }
  }
  ...
  if (value.includes('出社')) {
    ...
    if (timeMatch) {
      ...
      return {
        ...
        endTime: 1080, // 18:00
        memo: `${hour}:${minute.toString().padStart(2, '0')}出社`
      };
    }
  }
...
function generatePendingSchedules() {
  ...
  for (let rowIndex = 3; rowIndex <= 195; rowIndex++) {
    ...
    for (let day = 1; day <= 31; day++) {
      ...
      const schedule = parseTimePattern(cellValue);
...
console.log(`生成されたpending schedules: ${pendingSchedules.length}件`);

// JSON出力
fs.writeFileSync('/root/callstatus-app/pending_schedules_july_august.json', 
  JSON.stringify(pendingSchedules, null, 2));

console.log('ファイル出力完了: pending_schedules_july_august.json');
...
pendingSchedules.forEach(schedule => {
  ...
  if (!stats[month][schedule.status]) stats[month][schedule.status] = 0;
  stats[month][schedule.status]++;
});
